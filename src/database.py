...
class DatabaseManager:
    # ... (기존 __init__, connect, _ensure_connection 등은 그대로 유지) ...
    def __init__(self):
        ...
        with self.lock:
            ...
            try:
                ...
                with self.conn.cursor() as cursor:
                    sql = """
                        UPDATE game_status 
                        SET word_currect_count = %s, word_fail_count = %s,
                            end_at = NOW(), end_word = %s, end_platform = %s, end_user = %s
                        WHERE num = %s
                    """
            ...
            try:
                with self.conn.cursor() as cursor:
                    ...
                    if result and result[0] and str(result[0]).strip():
                        return str(result[0]), str(result[1]) if result[1] else None
                    else:
                        return "시작", None
            except Exception as e:
            ...
            try:
                with self.conn.cursor() as cursor:
                    sql_select = """
                        ...
                        WHERE is_use = FALSE AND can_use = TRUE AND available = TRUE 
    ...
    def log_system(self, level, source, message, trace=None):
        with self.lock:
            ...
            except Exception as e: print(f"[DB 로그 저장 실패] {e}")
    ...
    def log_history(self, nickname, input_word, previous_word, status, reason=None):
        with self.lock:
            ...
            except Exception as e: print(f"[DB 히스토리 저장 실패] {e}")
